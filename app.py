    ...
    if request.method == 'POST':
        try:
            ...
            if not data or 'title' not in data or 'description' not in data:
                return add_cache_headers(jsonify({'success': False, 'error': '缺少必要字段: title 或 description'}), 400)
...
def api_requirement_detail(req_id):
    ...
    if request.method == 'GET':
        ...
        if any(key in data for key in ['estimated_business_value', 'estimated_user_value', 'estimated_technical_value', 'estimated_effort']):
            requirement.estimated_business_value = data.get('estimated_business_value', requirement.estimated_business_value)
            requirement.estimated_user_value = data.get('estimated_user_value', requirement.estimated_user_value)
            requirement.estimated_technical_value = data.get('estimated_technical_value', requirement.estimated_technical_value)
            requirement.estimated_effort = data.get('estimated_effort', requirement.estimated_effort)
...
# 添加获取项目所有需求的API（用于路线图规划）
@app.route('/api/project/<int:project_id>/requirements', methods=['GET'])
@login_required
def api_project_requirements(project_id):
    """获取项目所有需求"""
    requirements = Requirement.query.filter_by(project_id=project_id).all()
    result = []
    for r in requirements:
...
if __name__ == '__main__':
    app.run(debug=True, host='0.0.0.0', port=5000)# app.py - 修复版（解决里程碑与需求管理问题）
from flask import Flask, render_template, request, jsonify, redirect, url_for, make_response, session, flash
from datetime import datetime
from database import db, init_db
from models import Project, Stakeholder, Requirement, Milestone
    ...
    try:
        ...
        for r in requirements:
            req_data = {
                ...
                'estimated_technical_value': r.estimated_technical_value if r.estimated_technical_value else 5,
                'estimated_effort': r.estimated_effort if r.estimated_effort else 5,
                'estimated_roi': float(r.estimated_roi) if r.estimated_roi is not None else 0,
                'actual_roi': float(r.actual_roi) if r.actual_roi is not None else 0,
                'value_assessor': r.value_assessor if r.value_assessor else '',
                'created_at': r.created_at.isoformat() if r.created_at else None
            }
            result.append(req_data)
